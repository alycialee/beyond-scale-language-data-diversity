...
def save_tokenizer_json(vocab, filename):
    ...
    tokenizer_config = {"version": "1.0", "truncation": None, "padding": None, "added_tokens": [
        {"id": 0, "special": True, "content": "[endoftext]", "single_word": False, "lstrip": False, "rstrip": False,
         "normalized": False}], "normalizer": None, "pre_tokenizer": {"type": "Whitespace"}, "post_processor": None,
                        "decoder": None, "model": {"type": "WordLevel", "vocab": vocab_dict, "unk_token": "<unk>"}}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_temp', type=float, default=10.0,
                        help="temperature on distribution of start hidden states")
    parser.add_argument('--transition_temp', type=float, default=0.1,
                        help="temperature on transition probability matrices")
    parser.add_argument('--skip_resample', action='store_true', help="whether to re-sample the dataset")
    parser.add_argument('--n_symbols', type=int, default=13, help="number of symbols")
    parser.add_argument('--n_values', type=int, default=12, help="number of entities (values)")
    parser.add_argument('--n_slots', type=int, default=11, help="number of properties (slots)")
    parser.add_argument('--value_identity_coeff', type=float, default=0.8,
                        help="mixing coefficient for identity matrix on value transition matrix")
    parser.add_argument('--random_data', action='store_true', help="whether to generate random data (not HMM)")
    parser.add_argument('--no_prior_slots', action='store_true',
                        help="only one slot transition matrix instead of a family")
    parser.add_argument('--prior_values', action='store_true',
                        help="use prior on values (many value transition matrices)")
    ...
    parser.add_argument('--seed', type=int, default=1111, help="seed to use for the dataset")
    parser.add_argument('--skip_all_generation', action='store_true', help="whether to skip everything")
    parser.add_argument('--root', type=str, default='../ginc-output-repro/', help="root dir")
